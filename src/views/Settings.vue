<template>
    <div class="min-h-screen w-screen dark:bg-gray-800 settings">
        <div class="p-2 sticky w-screen rounded-b-md shadow-md bg-gray-300 dark:bg-gray-600 dark:text-white text-ellipsis whitespace-nowrap flex justify-between items-center">
        ...
        <div class="container m-auto">
            ...
            <div class="dark:text-gray-200 p-4 text-lg">
                <label for="fontSize" class="flex justify-between items-center w-full">
                    ...
                    <div class="flex items-center justify-end w-60">
                        <button class="set-btn" @click="fontSize--; setFontSize()"><span class="material-symbols-rounded block">remove</span></button>
                        ...
                        <button class="set-btn" @click="fontSize++; setFontSize()"><span class="material-symbols-rounded block">add</span></button>
                    </div>
                </label>
            </div>
            <div class="dark:text-gray-200 p-4 text-lg" v-if="!autoscroll">
                <label for="columns" class="flex justify-between items-center w-full">
                    ...
                    <div class="flex items-center justify-end w-60">
                        <button class="set-btn" @click="columns--; setColumns()"><span class="material-symbols-rounded block">remove</span></button>
                        ...
                        <button class="set-btn" @click="columns++; setColumns()"><span class="material-symbols-rounded block">add</span></button>
                    </div>
                </label>
            </div>
...
const guitarMode = ref(store.state.guitarMode)
const chordMode = ref<'guitar' | 'ukulele'>(store.state.chordMode)
const darkMode = ref(store.state.darkTheme)
const fontSize = ref(store.state.fontSize)
const columns = ref(store.state.columnCount)
const showExplicit = ref(store.state.showExplicit)
const autoscroll = ref(store.state.autoscroll)

const isOnline = computed(() => navigator.onLine)
const availOffline = computed(() => store.state.songs.length > 0)

const { escape } = useMagicKeys()

watch(escape, (v) => v && close())


onMounted(() => {
  store.commit('install')
  setTimeout(() => {
    listAllSuggestions()
  }, 500)
})
...
function setColumns ():void {
  ...
  store.commit('setColumns', columns.value)
}

function close ():void {
  store.commit('save', availOffline.value)
  router.back()
}
...
function importViews (e: Event): void {
...
.set-btn {
  @apply bg-gray-400 dark:bg-gray-600 outline-none rounded-full hover:bg-gray-500 flex items-center justify-center w-6 h-6 text-xs;
}
</style>
