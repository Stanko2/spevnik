            ...
            >
                <div class="w-full h-10" v-if="showBar">
                    ...
                    <div class="mt-3" v-else-if="searchQuery.length > 0">
                        <p class="text-red-600">Nič sa nenašlo</p>
                    </div>
                </div>
            </transition>
        </div>
...
export default class SearchView extends Vue {
    ...
    @Watch('searchQuery')
    search ():void {
      this.results = this.findResults(this.searchQuery.toLocaleLowerCase()).splice(0, 10)
    ...
    findResults (query:string): SearchResult[] {
      ...
      for (const song of this.songs) {
        if (song.name.toLocaleLowerCase().includes(query)) {
          res.push({
            name: song.name,
            author: song.author,
            matchType: 'name',
        ...
        } else if (song.author.toLocaleLowerCase().includes(query)) {
          res.push({
            name: song.name,
            author: song.author,
            matchType: 'author',
        ...
        } else if (song.text.toLocaleLowerCase().includes(query)) {
          res.push({
            name: song.name,
            author: song.author,
            matchType: 'text',
