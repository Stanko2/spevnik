<template>
    <div class="h-full z-50 grid shadow-2xl" style="grid-template-rows: min-content 1fr min-content;">
        <div>
          <h1 class="text-2xl mt-5">Obsah</h1>
          <div class="flex justify-items-stretch h-10 z-30 m-3">
              <input type="search" class="dark:bg-gray-500 bg-gray-300 p-2 rounded-l-md outline-none w-10/12 grow" v-model="localSearchQuery">
              <button class="m-0 searchBtn shrink h-10 w-10" @click="selectMode(selectedMode)" style="border-radius: 0;">
                  <span class="material-symbols-rounded block">
                      search
                  </span>
              </button>
              <button class="m-0 searchBtn rounded-r-md shrink h-10 w-10" @click="showModeDropdown = !showModeDropdown">
                  <span class="material-symbols-rounded block">
                      {{ selectedMode?.icon || 'sort' }}
                  </span>
              </button>
          </div>
          <transition
              enter-active-class="duration-300 transition-all ease-in-out transform-gpu"
              leave-active-class="duration-300 transition-all ease-in-out transform-gpu"
              enter-from-class="-translate-y-5 opacity-0 z-10"
              leave-from-class="translate-y-0 opacity-100"
              enter-to-class="translate-y-0 opacity-100"
              leave-to-class="-translate-y-5 opacity-0 z-10"
          >
              <div v-if="showModeDropdown" class="absolute w-60 text-left right-2 bg-gray-300 dark:bg-gray-600 shadow-xl rounded-md z-50">
                  <div v-for="(mode,i) in songTreeModes" @click="selectMode(mode)" :key="i" class="transition-all hover:bg-gray-400 dark:hover:bg-gray-500 p-2 rounded-md flex items-center"><span class="material-symbols-rounded mr-1">{{ mode.icon }}</span>{{mode.text}}</div>
              </div>
          </transition>
        </div>
        <div class="overflow-y-auto xl:left-2/3 content mx-3">
          <Summary class="left-3 right-3" :nodes="nodes" v-if="store.state.songs.length > 0"></Summary>
          <div v-else class="text-red-400">Pre zobrazenie obsahu zapni offline režim v nastaveniach</div>
        </div>
        <div class="z-20 w-full bg-gray-600 rounded-b-md flex" v-if="loggedIn">
          <button class="btn w-1/2 flex items-center justify-between px-2" @click="openEditor(false)">
            {{ store.state.isAdmin ? 'Zmeniť aktuálnu pesničku' : 'Navrhnúť zmenu' }}
            <span class="material-symbols-rounded">
              edit
            </span>
          </button>
          <button class="btn w-1/2 flex items-center justify-between px-2" @click="openEditor(true)" v-if="store.state.songs.length > 0">
            {{ store.state.isAdmin ? 'Pridať novú pesničku' : 'Navrhnúť novú pesničku' }}
            <span class="material-symbols-rounded">
              add
            </span>
          </button>
        </div>
    </div>
</template>

<script lang="ts" setup>
import { Song } from '@/store'
import { ref, watch, onMounted, nextTick } from 'vue'
import { Search } from './Search'
import Summary, { SongTreeNode } from './Summary'
import { useStore } from 'vuex'
import { useRouter } from 'vue-router'
import { useMagicKeys } from '@vueuse/core'

interface SortMode {
    name: string
    icon: string
    text: string
}

const { q, e } = useMagicKeys()
watch(q, (v) => v && openEditor(true))
watch(e, (v) => v && openEditor(false))

const props = defineProps({
  SearchQuery: { type: String, default: '' }
})

const localSearchQuery = ref(props.SearchQuery)
watch(() => props.SearchQuery, (newQuery) => {
  localSearchQuery.value = newQuery
})

onMounted(() => {
  nextTick().then(() => {
    selectMode(songTreeModes[0])
  })
  loggedIn.value = store.state.credential?.uid != undefined
})

const router = useRouter()
const store = useStore()

const loggedIn = ref(store.state.credential?.uid != undefined)
const songTree = ref<SongTreeNode | null>(null)
const showModeDropdown = ref(false)
const selectedMode = ref<SortMode>({ name: 'default', text: 'Podľa spevníkov', icon: 'book' })
const nodes = ref<SongTreeNode[]>([])

function openEditor (newSong:boolean):void {
  if (newSong) {
    router.push({
      name: 'Editor',
      params: {
        id: '-1'
      }
    })
  } else { router.push({ path: `/edit/${store.state.currentSong}` }) }
}

function selectMode (mode: SortMode):void {
  selectedMode.value = mode
  showModeDropdown.value = false
  let songs: Song[] = store.state.songs
  if (!store.state.showExplicit) {
    songs = songs.filter(song => !song.explicit)
  }
  const songsFiltered: Song[] = Search(localSearchQuery.value).map(s => {
    const song = songs.find(song => song.id === s.id)
    if (song) {
      return song
    }
    throw new Error(`Song with id: ${s.id} not found`)
  })
  nodes.value = buildSongTree(songsFiltered)
}

function buildSongTree (songs: Song[]):SongTreeNode[] {
  switch (selectedMode.value.name) {
    case 'default':
      return buildDefaultSongTree(songs)
    case 'author':
      return buildSongTreeByAuthor(songs)
    case 'alphabetical':
      return songs.sort((a:Song, b:Song) => a.name.localeCompare(b.name)).map((song:Song) => ({
        id: song.id,
        name: song.name,
        type: 'leaf',
        explicit: song.explicit
      })) as SongTreeNode[]
    case 'popular':
      return songs.sort((a:Song, b:Song) => (b.views ?? 0) - (a.views ?? 0)).map((song:Song) => ({
        id: song.id,
        name: song.name,
        type: 'leaf',
        explicit: song.explicit
      })) as SongTreeNode[]
    default:
      throw new Error('Unknown sort mode')
  }
}

function buildSongTreeByAuthor (songs: Song[]): SongTreeNode[] {
  const authors = songs.map(song => song.author).filter((author, i, arr) => arr.indexOf(author) === i)
  const tree: SongTreeNode = {
    id: -1,
    name: '',
    type: 'node',
    children: []
  }
  for (const author of authors) {
    const songsByAuthor = songs.filter(song => song.author === author)
    tree.children.push({
      id: -1,
      name: author,
      type: 'node',
      children: (songsByAuthor.map((song:Song) => ({
        id: song.id,
        name: song.name,
        type: 'leaf',
        explicit: song.explicit
      })) as SongTreeNode[]),
      expanded: false
    })
  }
  sortTree(tree)
  return tree.children as SongTreeNode[]
}

function buildDefaultSongTree (songs: Song[]):SongTreeNode[] {
  const tree: SongTreeNode = {
    children: [],
    id: -1,
    name: '',
    type: 'node',
    expanded: false
  }
  for (const song of songs) {
    addSongToTree(tree, song)
  }
  sortTree(tree)
  return tree.children
}

function addSongToTree (tree: SongTreeNode, song: Song): SongTreeNode {
  const path = song.path.split('/')
  path.pop()
  let currentNode = tree
  for (const part of path) {
    const found = currentNode.children.find((node: SongTreeNode) => node.name === part)
    if (found) {
      currentNode = found
    } else {
      const newNode: SongTreeNode = {
        id: -1,
        name: part,
        type: 'node',
        children: [],
        expanded: false
      }
      currentNode.children.push(newNode)
      currentNode = newNode
    }
  }

  currentNode.children.push({
    id: song.id,
    name: song.name,
    type: 'leaf',
    children: [],
    explicit: song.explicit
  })

  return tree
}

function sortTree (tree: SongTreeNode):void {
  if (tree.type === 'leaf') return
  tree.children.sort((a: SongTreeNode, b:SongTreeNode) => {
    const aNum = parseInt(a.name.split('.')[0])
    const bNum = parseInt(b.name.split('.')[0])
    if (aNum !== bNum && !isNaN(aNum) && !isNaN(bNum)) {
      return aNum > bNum ? 1 : -1
    } else {
      return a.name.localeCompare(b.name)
    }
  })
  for (const child of tree.children) {
    sortTree(child)
  }
}

const songTreeModes: SortMode[] = [
  {
    name: 'default',
    text: 'Podľa spevníkov',
    icon: 'book'
  },
  {
    name: 'popular',
    text: 'Najpopulárnejšie',
    icon: 'sort'
  },
  {
    name: 'author',
    text: 'Podľa interpretov',
    icon: 'person'
  },
  {
    name: 'alphabetical',
    text: 'Abecedne',
    icon: 'abc'
  }
]
</script>

<style scoped>
@reference 'tailwindcss';

.content::-webkit-scrollbar{
    display: block;
    width: 15px;
    background-color: transparent;
    position: absolute;
}

.searchBtn {
  @apply p-2 outline-none grow flex justify-center items-center bg-gray-800 text-white w-10;
}

.content::-webkit-scrollbar-thumb{
    @apply bg-gray-400 dark:bg-gray-200 rounded-full hover:bg-gray-500;
}
</style>
